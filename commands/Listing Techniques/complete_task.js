...
module.exports = {
  ...
  aliases: [
    ...
    "complete135tasks",
    "complete1-3-5tasks",
  ], //Psuedo-command that allows activation of code
  usage:
    "Use '!complete135task medium 2' to remove the second medium task off of the user's 1-3-5 list.",
  async execute(message, args) {
    await mongodb1().then(async (mongoose) => {
      try {
        var counter = 0;
        var listOftasks =
          "Please specify the number corresponding to the task that you have completed. Type '!complete135task <category: small, medium, big> <task number>'.:smile: \n";
        ...
        if (
          taskings["bigTask"] == "" &&
          taskings["mediumTasks"] == [] &&
          taskings["littleTasks"] == []
        ) {
          ...
          for (let i = 0; i < taskings["mediumTasks"].length; i++) {
            counter += 1;
            listOftasks =
              listOftasks + `${i + 1}: ${taskings["mediumTasks"][i]}. \n`;
          }
          listOftasks = listOftasks + `\nSmall tasks: \n `;
          for (let i = 0; i < taskings["littleTasks"].length; i++) {
            counter += 1;
            listOftasks =
              listOftasks + `${i + 1}: ${taskings["littleTasks"][i]}. \n`;
          }

          if (args[0] != "small" && args[0] != "medium" && args[0] != "big") {
            return message.channel.send(
              "Please specify only 'small', 'medium' or 'large' category. Type '!complete135task <category: small, medium, big> <task number>'.:smile: \n"
            );
          }
          if (parseInt(args[1]) > counter) {
            return message.channel.send(
              "Please specify a number that corresponds to a task above. Type '!complete135task <category: small, medium, big> <task number>'.:smile: \n"
            );
          }
          ...
          if (counter != -1) {
            if (args[0] == "small") {
              littleTaskings.splice(args[1] - 1, 1);
            } else if (args[0] == "medium") {
              mediumTaskings.splice(args[1] - 1, 1);
            } else if (args[0] == "big") {
              bigTasking = "";
            }
          }
          ...
          message.channel.send(
            `${args[0]} task, ${args[1]}, has been successful completed! You may use \'!list135\' to view your updated 1-3-5 list.`
          );
        }
      } finally {
